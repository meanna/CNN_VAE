...
def image_generation():
    ...
    # Plot
    # save_each_image(generated.cpu())
    # show_image_grid(generated.cpu())
    # f = plt.figure(figsize=(10, 10))
    # # plt.title(str(target_attr))
    # ax = f.add_subplot(1, 1, 1)
    # ax.imshow(convert_batch_to_image_grid(generated.cpu()))
    # plt.axis('off')
    #
    # plt.savefig(os.path.join(save_dir, "Results", "generation.png"), dpi=200, bbox_inches='tight')
    #
    # plt.show()
    # plt.clf()


def image_generation_zero():
    batch = 16
    latent_dim = vae_net.latent_dim
    # sample both initial input and condition
...
def image_generation_with_condition(test_labels):
    ...
    logits = vae_net.decoder(z_cond)
    generated = torch.sigmoid(logits)
    save_path = os.path.join(result_folder, "generation_with_cond.png")
    vutils.save_image(generated, save_path)
    print("save image at", save_path)

    save_path_ori = os.path.join(result_folder, "ori.png")
...
def image_generation_clip(target_attr=None, save_path=None):
    """
    ...
    if target_attr:
        ...
        labels = text_features.to(device)
    ...
    save_path_ori = os.path.join(result_folder, "generation_" + prompt + ".png")
    vutils.save_image(generated, save_path_ori)
    print("save image at", save_path_ori)
...
if __name__ == "__main__":
    if run_train:
        train()
    image_generation_clip(target_attr="sad", save_path=result_folder)
    ...
    #
    # image_generation_with_condition(test_labels)
    # image_generation()
    # image_generation_zero()
